from GreyRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from GreyRobot import DRAGONS
...
async def grey_is_admin(chat_id: int):
    status = False
    grey = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if grey.id == user.id:
            status = True
